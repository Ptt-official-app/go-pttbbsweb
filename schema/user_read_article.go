...
func UpdateUserReadArticles(userReadArticles []*UserReadArticle, updateNanoTS types.NanoTS) (err error) {
	...
	for idx, each := range userReadArticles {
		query := &UserReadArticleQuery{UserID: each.UserID, ArticleID: each.ArticleID}
...
func FindUserReadArticles(userID bbs.UUserID, articleIDs []bbs.ArticleID) ([]*UserReadArticle, error) {
